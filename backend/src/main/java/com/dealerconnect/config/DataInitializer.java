package com.dealerconnect.config;
...
public class DataInitializer implements CommandLineRunner {
    ...
    private static final String ADMIN_EMAIL        = "admin@dealerconnect.com";
    private static final String SUPER_ADMIN_EMAIL  = "superadmin@dealerconnect.com";
    ...
    public void run(String... args) {
        ...
        // Rebranding Fix: Find admin by email OR by code EMP001
        Optional<Employee> existingAdmin = employeeRepo.findByEmail(ADMIN_EMAIL);
        if (existingAdmin.isEmpty()) {
            existingAdmin = employeeRepo.findByEmployeeCode("EMP001");
        }

        existingAdmin.ifPresent(emp -> {
            boolean updated = false;
            // Update email to new domain if needed
            if (!ADMIN_EMAIL.equals(emp.getEmail())) {
                log.info("DataInitializer: Updating admin email from {} to {}", emp.getEmail(), ADMIN_EMAIL);
                emp.setEmail(ADMIN_EMAIL);
                updated = true;
            }
            ...
            if (updated) {
                ...
                log.info("DataInitializer: admin record updated ✓");
            } else {
                log.info("DataInitializer: admin record OK.");
            }
        });

        // Seed Super Admin if not present by either email or employee code
        Optional<Employee> superAdminByEmail = employeeRepo.findByEmail(SUPER_ADMIN_EMAIL);
        Optional<Employee> superAdminByCode = employeeRepo.findByEmployeeCode("SA001");

        if (superAdminByCode.isPresent()) {
            Employee sa = superAdminByCode.get();
            boolean updated = false;
            
            if (!SUPER_ADMIN_EMAIL.equals(sa.getEmail())) {
                log.info("DataInitializer: Updating SuperAdmin email from {} to {}", sa.getEmail(), SUPER_ADMIN_EMAIL);
                sa.setEmail(SUPER_ADMIN_EMAIL);
                updated = true;
            }
            
            if (!passwordEncoder.matches(SUPER_ADMIN_PWD, sa.getPasswordHash())) {
                log.warn("DataInitializer: SuperAdmin password hash is wrong – resetting …");
                sa.setPasswordHash(passwordEncoder.encode(SUPER_ADMIN_PWD));
                updated = true;
            }
            
            if (updated) {
                employeeRepo.save(sa);
                log.info("DataInitializer: SuperAdmin record updated ✓");
            }
        } else if (superAdminByEmail.isEmpty()) {
            seedSuperAdmin();
        }
    ...
    private void seedSuperAdmin() {
        ...
        Employee superAdmin = employeeRepo.findByEmployeeCode("SA001")
            .orElse(Employee.builder().employeeCode("SA001").build());
            
        superAdmin.setFirstName("DMS");
    ...
    private void seedSuppliersAndParts() {
        ...
        // ── Suppliers ────────────────────────────────────────────────────────
        Supplier hyundai = supplierRepo.save(Supplier.builder().name("Genuine DealerConnect Spares").contactName("Anil Kumar").phone("9845012345").email("sales@dealerconnectspares.com").address("Chennai, TN").build());
